...
def test_aim_engine(sample_data):
    source_data, domain = sample_data
    engine = AIMEngine(synth_epsilon=10, proc_epsilon=.1, gen_kwargs=dict(n_iters=10, rounds=1))
    engine.fit(source_data, domain=domain)
    synthetic_data = engine.generate(n_records=source_data.shape[0])
    validate_synthetic_data(synthetic_data, source_data, domain)

def test_priv_bayes_engine(sample_data):
    source_data, domain = sample_data
    engine = PrivBayesEngine(synth_epsilon=10, proc_epsilon=.1, gen_kwargs=dict(n_iters=10))
    engine.fit(source_data, domain=domain)
    synthetic_data = engine.generate(n_records=source_data.shape[0])
    validate_synthetic_data(synthetic_data, source_data, domain)

def test_mst_engine(sample_data):
    source_data, domain = sample_data
    engine = MSTEngine(synth_epsilon=10, proc_epsilon=.1, gen_kwargs=dict(n_iters=10))
    engine.fit(source_data, domain=domain)
    synthetic_data = engine.generate(n_records=source_data.shape[0])
    validate_synthetic_data(synthetic_data, source_data, domain)
