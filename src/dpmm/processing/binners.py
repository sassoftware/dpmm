...
from dpmm.processing.utils import approx_bounds, approx_quantile, optimal_n_bins
...
class Binner:
    ...
    def __init__(
        ...
        epsilon=None,
        max_n_bins=None,
        rnd: RandomState = None,
        structural_zeros: List[Tuple] = None,
    ):
        self.n_bins = n_bins
        self.lower = lower
        self.upper = upper
    ...
    def get_bounds(self, arr, recompute=False, epsilon=None):
        if self.has_bounds:
            if (epsilon is not None) and recompute:
                min_, max_ = [
                    approx_quantile(
                        arr,
                        alpha=q,
                        epsilon=self.spend_epsilon(epsilon / 2),
                        lower=self.lower,
                        upper=self.upper,
                        rnd=self.rnd,
                    )
...
class UniformBinner(Binner):
    ...
    def apply_log(self, arr):
        return np.sign(arr) * np.log(1 + np.absolute(arr))

    def undo_log(self, arr):
        return np.sign(arr) * (np.exp(arr) - 1)

    def fit(self, arr: np.ndarray):
        if self.log:
            arr = self.apply_log(arr)
    ...
    def get_intervals(self, bin_idx):
    ...
    def inverse_transform(self, arr: np.ndarray) -> np.ndarray:
        ...
        if self.log:
            i_series = self.undo_log(i_series)
...
class PrivTreeBinner(UniformBinner):
    ...
    def priv_tree(self, data: np.ndarray, eps=None):
        if len(data.shape) == 1:
