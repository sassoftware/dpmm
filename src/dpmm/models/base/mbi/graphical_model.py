# Adapted from: https://github.com/ryan112358/private-pgm/blob/1da21c8b38149b05f1385b8e54116568b700b4fa/src/mbi/graphical_model.py
...
from dpmm.models.base.mbi import Domain, Dataset, CliqueVector, JunctionTree


class GraphicalModel:
    def __init__(self, domain, cliques, total=1.0, elimination_order=None, prng=None):
        ...
        self.domain = domain
        self.total = total
        tree = JunctionTree(domain, cliques, elimination_order, prng=prng)
    ...
    def set_random_state(self, prng):
        if prng is not None:
            self._prng = prng
            self.junction_tree.set_random_state(prng)
    ...
    def synthetic_data(  # noqa: C901
        ...
        if condition_records is not None:
            total = condition_records.shape[0]
        ...
        if condition_records is not None:
            df = condition_records.reindex(columns=self.domain.attrs, fill_value=0)
        else:
        ...
        def synthetic_col(counts, total):
            if method == "sample":
                ...
                return self.prng.choice(counts.size, total, True, probas)
            counts *= total / counts.sum()
            frac, integ = np.modf(counts)
            integ = integ.astype(int)
            extra = total - integ.sum()
            if extra > 0:
                idx = self.prng.choice(counts.size, extra, False, frac / frac.sum())
                integ[idx] += 1
            vals = np.repeat(np.arange(counts.size), integ)
            self.prng.shuffle(vals)
            return vals
        ...
        if condition_records is None:
            ...
            used = set(condition_records.columns)
            order = [col for col in order if col not in used]
